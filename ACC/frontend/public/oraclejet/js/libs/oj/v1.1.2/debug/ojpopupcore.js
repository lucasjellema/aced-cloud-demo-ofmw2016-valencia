...
oj.PopupServiceImpl.prototype.open = function (options)
{
  ...
  oj.Assert.assertStringOrNull(modality);
  if (!modality ||
  ...
  oj.ZOrderUtils.addToAncestorLayer(popup, launcher, events, modality, layerClass);
...
{
  /** @type {jQuery} */
  var defaultLayer = $("#" + oj.ZOrderUtils._DEFAULT_LAYER_ID);
 ...
 * @public
 */
oj.ZOrderUtils.addToAncestorLayer = function (popup, launcher, events, modality, layerClass)
{
  ...
  var ancestorLayer = oj.ZOrderUtils.getFirstAncestorLayer(modality !== oj.PopupService.MODALITY.NONE ? null : launcher);
...
oj.PopupLiveRegion.prototype.destroy = function ()
{
  if (!isNaN(oj.PopupLiveRegion._refCounter))
  {
    ...
    {
      var liveRegion = $("#" + oj.PopupLiveRegion._POPUP_LIVE_REGION_ID);
...
{
  // bug 21115861 - removed the rule due to last minute issues uncovered prior to v1.1.
  return false;
};
