 ...
 * @private
 */
oj.DefaultNavListHandler.prototype.UpdateActiveDescendant = function (elem) {
...
{
    //TOO: When using tree data source, item is being passed as jquery object instead of element. 
    return $(item)[0].hasAttribute("aria-selected");
};
...
oj.HorizontalNavListHandler.prototype.BeforeRenderComplete = function () {
    ...
    this.m_widget.element.find('.' + this.m_widget.getItemElementStyleClass()).each(function (index) {
        if (index > 0) {
            $(this).before('<li role="separator" class="' + self.m_widget.DIVIDER_STYLE_CLASS + '"></li>');//@HTMLUpdateOK 
        }
    });
};
...
{
    return $(item)[0].hasAttribute("aria-pressed");
};
...
oj.SlidingNavListHandler.prototype.Expand = function (groupItem, animate) {
    ...
    if (currentListRoot.length > 0) {
        ...
        //Ref bug 21029080, During initialization, listview expands child before parent which is causing the issue.
...
oj.SlidingNavListHandler.prototype.Collapse = function (target, animate) {
    ...
    target.siblings().attr('role', 'menuitem');
...
oj.SlidingNavListHandler.prototype.UpdateAriaPropertiesOnSelectedItem = function (elem, highlight) {
    var itemContent = this.m_widget.getItemContentElement(elem);
    if (highlight)
        itemContent.attr('aria-describedby', 'selectedLabel');
    else
        itemContent.removeAttr('aria-describedby');
};
/**
 * Determines whether the specified item is expanded
 * @param {jQuery} item the item element
...
oj.SlidingNavListHandler.prototype.GetState = function (item) {
    var expanded = item.children('.' + this.m_widget.getGroupItemStyleClass())
            .children('.' + this.m_widget.ITEM_CONTENT_STYLE_CLASS)
            .attr("aria-expanded");
    if (expanded === "true")
    {
...
oj.SlidingNavListHandler.prototype.SetState = function (item, state)
{
    ...
    {
        item.children('.' + this.m_widget.getGroupItemStyleClass())
                .children('.' + this.m_widget.ITEM_CONTENT_STYLE_CLASS)
...
oj.SlidingNavListHandler.prototype.UpdateActiveDescendant = function (elem) {
    this.m_widget.element.attr("aria-activedescendant", this.m_widget.getItemContentElement(elem).attr('id'));
};

oj.SlidingNavListHandler.prototype.ModifyListItem = function ($item, itemContent) {
    itemContent.attr('role', 'menuitem');
 ...
 */
oj.SlidingNavListHandler.prototype._buildSlidingNavListHeader = function ()
{
    ...
    vSeparator.attr('role', 'separator')
    ...
    this._toolbar.append(this._previousLink)// @HTMLUpdateOK
            .append(vSeparator)// @HTMLUpdateOK
...
/** @lends _ojNavigationListView.prototype */
{
    ...
    */
    ...
    {
        this.m_listHandler.UpdateActiveDescendant(elem);
    },
    /**
    ...
    HighlightActive: function()
    {
        if(this.element.is(':focus')) {
            _ojNavigationListView.superclass.HighlightActive.apply(this, arguments);
        }
    },
    ...
    HandleMouseClick: function (event) {
        ...
        //TODO Bug 20993095, once it is fixd in listview probably don't need to override HandleMouseClick
    ...
    HandleKeyDown: function (event) {
        ...
        this.m_listHandler.HandleExpandAndCollapseKeys(event, keyCode, current, currentItemKey);
    ...
    HighlightUnhighlightElem: function (elem, style, highlight)
    {
        ...
        if (style === "oj-selected") {
            this.m_listHandler.UpdateAriaPropertiesOnSelectedItem(elem, highlight);
        }
        ...
        if (elem.hasClass('oj-selected') || elem.hasClass('oj-hover') || elem.hasClass('oj-disabled')) {
    ...
    setOptions: function (options, flags)
    {
        if (options['selection'] !== undefined) {
            ...
            if (!!value) {
                ...
                if (!selection || selection.length === 0 || selection[0] !== value) {
                    var item = this.FindElementByKey(value);
                    //TODO: This will not prevent from firing option change event. do we need to avoid firing option change event?
                    if (this.IsSelectable(item)) {
                        var shouldSelect = this._fireBeforeSelectEvent(null, $(item));
                        if (shouldSelect) {
                            return _ojNavigationListView.superclass.setOptions.call(this, {'selection': [value]}, flags);
                        }
                    }
                }
            } else {
                return _ojNavigationListView.superclass.setOptions.call(this, {'selection': []}, flags);
            }
        } else {
            return _ojNavigationListView.superclass.setOptions.call(this, options, flags);
        }
        return false;
    },
    /**
    ...
    itemRenderComplete: function(elem, context)
    {
        ...
        if (groupItem.length > 0) {
            ...
            groupIcon.insertAfter(groupItem.children('.' + this._ITEM_LABEL_STYLE_CLASS));// @HTMLUpdateOK
            groupIcon.attr('aria-hidden', 'true');
            if ($item.hasClass('oj-disabled')) {
        ...
        if (icon.length > 0) {
            ...
            if (this.ojContext.options['display'] === 'icons') {
                ...
                if (!icon.attr('title')) {
                    icon.attr('title', this.getItemLabel($item));
                }
            }
