 ...
 *   <tbody>
 *     <tr>
 *       <td>oj-button-half-chrome</td>
 *       <td><p>Used to give the button a "half-chrome" look.  
 *           In typical themes, half-chrome buttons have no chrome (background or border) 
 *           normally, but acquire chrome in their hover, active, and selected states. 
 *           Half-chroming is recommended for buttons in a toolbar.
 *
 *           <p>The class is applied as follows:
 * 
 *           <ul>
 *             <li>For buttons in a buttonset, the class must be applied to the buttonset element.</li>
 *             <li>For buttons not in a buttonset, the class can be applied to the button's root element, or to any ancestor, such as a toolbar.
 *                 See the <a href="#rootAttributes">rootAttributes</a> option.</li>
 *           </ul>
 * 
 *           <p>As long as those requirements are met, it is fine for the class to appear multiple times in the button's ancestry, inclusive.  
 *           E.g. the class can be applied both to a buttonset and to an ancestor such as a toolbar, where the latter takes care of buttons 
 *           under that ancestor that are not in the buttonset.</td>
 *     </tr>
 *     <tr>
 *       <td>oj-button-sm<br>
 *           oj-button-lg<br>
 ...
 *       <td>Draws attention to a push button, often identifies the primary action in a set of buttons.  Designed for use with a push button. </td>
 *     </tr>
 *     <tr>
...
oj.__registerWidget("oj.ojButton", $['oj']['baseComponent'],
{
    ...
    _destroy: function() // Override of protected base class method.  Method name needn't be quoted since is in externs.js.
    {
        ...
        // @HTMLUpdateOK reparent existing DOM
    ...
    _setLabelOnDomAtCreateTime: function() // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    {
        ...
        // @HTMLUpdateOK attach detached DOM created from trusted string and existing DOM
...
*/
    ...
    _showMenu: function(event, focus) // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
    {
        ...
        this._getMenu().open(event, {"launcher": this.element, "initialFocus": focus});
...
oj.__registerWidget("oj.ojButtonset", $['oj']['baseComponent'],
{
    ...
    _setOption: function( key, value, flags ) // Override of protected base class method.  Method name needn't be quoted since is in externs.js.
    {
        ...
        // best to call super at end, so that optionChange (fired at end of super) is fired at very end
