...
DvtAxisRenderer._getGroupAxisPreferredSize = function(axis, axisInfo, size, availSize, bHoriz) {
  ...
  var gap = numLevels > 1 ? DvtAxisDefaults.getGapSize(axis, axis.getOptions()['layout'][gapName]) : 0;
  for (var level = 0; level < numLevels; level++) { // allocate space outermost to innermost
    ...
    if (bHoriz) {
      var maxDims = DvtTextUtils.getMaxTextDimensions(labels);
      labelSize = axisInfo.isLabelRotated(level) ? Math.ceil(maxDims.w) : maxDims.h;
    }
    else
...
DvtAxisInfo.prototype.SkipTangentialLabels = function(labels, labelDims) {
  ...
  if (numLabels > 1) {
    ...
    if (!this.IsOverlappingDims(prevLabelDims, labelDims[numLabels - 1]) && !this.IsOverlappingDims(labelDims[numLabels - 1], labelDims[0]))
