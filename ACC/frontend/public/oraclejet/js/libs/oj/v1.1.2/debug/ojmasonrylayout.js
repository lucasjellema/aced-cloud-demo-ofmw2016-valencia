...
MasonryLayoutCommon.prototype.setup = function(init, reorder)
{
  ...
  {
    //FIX BUG 20346824: notify the peer that a layout cycle is starting so that
...
{
  //FIX BUG 20867069: when drag and drop reordering, the layoutPhase doesn't
...
oj.__registerWidget("oj.ojMasonryLayout", $['oj']['baseComponent'],
{
  ...
  {
    var mlCommon = this._mlCommon;
    //FIX BUG 19996416: if we're already in a layout cycle, finish it immediately
    ...
    //FIX BUG 20008576: if the style class isn't changing, don't do anything;
    //this prevents the layout from preparing for a resize and then waiting for
    //the end of a resize animation that won't happen
    //FIX BUG 20867456: throw error if style class isn't changing
  ...
  _setupReorderHandles: function()
  {
    var elem = this.element;
    var options = this.options;
    var reorderHandles = elem.find(options.reorderHandle);
    reorderHandles
      .attr("draggable", "true")
      .addClass("oj-draggable");
    elem
  ...
  {
    //FIX BUG 20867069: if the drop happens before the drag move layout 
    //animation is done, finish that layout cycle immdiately before processing
    //the drop
    var mlCommon = this._mlCommon;
    if (mlCommon.isInLayoutCycle())
      mlCommon.finishLayoutCycle();
          ...
          tileUnderPoint !== this._draggedTile)
      {
        ...
        if ((bRightSide && !this._bRTL) || (!bRightSide && this._bRTL))
        {
          ...
          else
          {
            ...
            elem.appendChild(this._dropSite);
          }
        }
        else
        {
