...
(function () 
{
  ...
  */
  ...
  var  /** @const */   OJ_DRAGGABLE      = "oj-draggable",     // node is draggable
       ...
       /** @const */   OJ_INVALID_DROP   = "oj-invalid-drop",  // over node that can't be dropped on
       /** @const */   OJ_DROP           = "oj-drop" ;         // nodes that are valid drop targets


  var  /** @const */   OJT_DROP_OK           = "oj-tree-drop-ok",
       /** @const */   OJT_DROP_INVALID      = "oj-tree-drop-invalid",
       /** @const */   OJT_DROP_MARKER       = "oj-tree-drop-marker",
       /** @const */   OJT_DROP_MARKER_LINE  = "oj-tree-drop-marker-line";
...
//          $ul.on("transitionend", this._proxyTransitionEndHandler) ;
     ...
     _getElemHeight : function( el)
     {
       ...
       this._$container[0].appendChild(div[0]) ;
...
//          $ul.on("transitionend", this._proxyTransitionEndHandler) ;
    ...
    /**
     ...
     {
        if (node.hasClass(OJ_COLLAPSED)) {                  // end collapse
...
//        $ul.css('overflow', 'hidden') ;        // from _expand()  
     ...
     */
     ...
     /** Creates a new node and adds it to the tree.  Triggers a "create" event.
       ...
       * @param {Object} data - An object literal containing data to create a new node.  The object properties
       *                       are the same as for defining a JSON node:<br>
       ...
       * </code>
       * @return {Object} Returns the jQuery wrapped node created from the 'data' argument.
       * @expose 
       * @public
       * @instance
       * @memberof oj.ojTree 
     ...
     {
        return this._createNode(refnode, position, data);
     },
     ...
     dehover : function()
     {
       ...
       p = p.removeClass(OJ_HOVER).parent();
...
//       }
     ...
     _ComponentCreate: function ()
     {
        ...
        if (this._animDuration) {
          ...
          this._isSafari = (ai.browser == "safari") ;
...
//   },
     ...
     /**
       *  Returns the widget instance for the supplied node as the member
...
//
     ...
     _handleDisabledChanged  : function(newval)
     {
        ...
        curState  = this._$container_ul.hasClass(OJ_DISABLED) ;
        if (! curState) {
...
//      this._emitEvent({});
     ...
     {
       var p  = {},         // new prepared_move object
           _t = this;
       ...
       if (!is_cb && this._data.core.prepared_move.o               &&
                     this._data.core.prepared_move.o[0] === p.o[0] &&
                     this._data.core.prepared_move.r[0] === p.r[0] &&
                     this._data.core.prepared_move.p    === p.p)  {
//        this._emitEvent(this._data.core.prepared_move, "prepare_move, true);
          if (cb)  {
...
//     this._emitEvent(this._data.core.prepared_move, "prepare_move", true);
     ...
     /**
       *  Checks the prepared_move object, and returns true if the move is
...
//           this._emitEvent({ "obj" : node, "type" : str}, "settype", true); 
      ...
      /**
        * Applies all necessary classes to the node and its descendants.
...
//       this._emitEvent({ "obj" : obj }, "cleanNode", true);
      },
      ...
      _createNode :  function (obj, position, js, callback, is_loaded)
      {
         ...
         if (!js["data"]) {
           js["data"] = (js["title"] !== undefined)? js["title"] :
                                                     this._getString(TRANSKEY_NEWNODE);
         }
         ...
         $.each(js["data"], function (i, m) {
             ...
             if (typeof m == "string")  {
               sp[0].textContent = m ;           // per security post-impl
               tmp.attr("href", "#");
             }
             else  {
               if (! m["attr"])  {
             ...
             tmp.append(sp) ;                                                                            //@HTMLUpdateOK
             tmp.prepend("<ins class='oj-tree-icon oj-tree-node-icon oj-component-icon'>&#160;</ins>");  //@HTMLUpdateOK
             ...
             d.append(tmp);                                                                             //@HTMLUpdateOK
         });

         d.prepend("<ins class='" + OJ_DISC + "'>&#160;</ins>");                                        //@HTMLUpdateOK
         if (obj === -1) {
...
//           }
    ...
    {
       var selectMode  = this._data.core.selectMode ;    // 0 = no select, 1 to n, or -1 = multi-select
       ...
       var isMultiple     = (selMultMod == "on" || (selMultMod !== false && e && oj.DomUtils.isMetaKeyPressed(e))),           
           isRange        = ((selRangeMod !== false) && (e &&  (e[selRangeMod + "Key"]))  &&
                            this._data.ui.lastSelected && this._data.ui.lastSelected[0] !== node[0] &&
                            (this._data.ui.lastSelected.parent()[0] === node.parent()[0])),
           proceed        = true,
           t              = this;
       ...
       if (check) {
          ...
          switch(!0)  {
             case (isRange):
                 ...
                 node = node[ node.index() < this._data.ui.lastSelected.index() ? "nextUntil" : "prevUntil" ](".oj-tree-last-selected").addBack();

                 if (selectMode == -1 || node.length < selectMode)  {
                   this._data.ui.lastSelected.removeClass("oj-tree-last-selected");
                   this._data.ui.selected.each(function ()
                                           {
                                              if (this !== t._data.ui.lastSelected[0])  {
                                                t._deselect(this);
                                              }
                                         });
                   isSelected = false;
             ...
             case  (this._data.ui.touchEvent && selectMode == -1) :  // touch and multi-select mode
                 this._data.ui.touchEvent = false ;                  // prevent race, when we call toggleselect()
             ...
             case (! isSelected && !isMultiple): 
                 if (e) {                                          // if mouse click
                   if (this._data.ui.selected && (this._data.ui.selected.length == 1)) {
                     this._deselect(this._data.ui.selected) ;      // clear the selection 
                   }
                   else {
                     this._deselectAll(this._data.ui.selected) ;   // clear all selections
                   }
                 }
             ...
             case (! isSelected && isMultiple): 
                 if ((selectMode == -1) ||
                     (this._data.ui.selected.length + 1 <= selectMode))  { 
                   proceed = true;
                 }
                 break;
          }
       ...
       if (proceed && !isSelected)  {
          if (!isRange)  {
            this._data.ui.lastSelected = node;
          }
          ...
          if (! this._data.core.suppressSelectEvent) {
...
//      if (obj && obj !== -1 && this._data.ds.progressiveUnload && ($.isFunction(s["data"]) || !!s["ajax"]))  {
     ...
     {
        var _this = this;
        this._load_node_json(obj,                             // node or -1 for tree
                             function ()                      // success func
                              {
                                _this._emitEvent({ "obj" : _this._getNode(obj) }, "load_node", true);
                                s_call.call(this);
     ...
     {
       var _this = this;
       this._load_node_html(obj, function ()                   // success func
                                   {
                                     _this._emitEvent({ "obj" : _this._getNode(obj)}, "load_node", true) ;
                                     s_call.call(this);
                                   },
     ...
     "saveSelected" : function ()
     {
        ...
        this._emitEvent(this._data.ui.to_select, "savedselected", true);
     },
...
*/
    ...
    /**
     ...
     /**
       *  Parses a JSON representation of a tree or tree node.  Returns a jQuery
       *  wrapped <ul> structure.
       *  @private
       */
...
//   this._data.dnd.targ_ml_width = this._data.dnd.targ_ml_width < 25? 25   : this._data.dnd.targ_ml_width ;
     ...
     /**
       *  Positions and shows (or hides) the drag insert position marker and line, and
       *  updates the drag and drop targets with the can/can't drop classes.
       *  @private
       */
     _dnd_show : function ()
     {
        var  dnd      = this._data.dnd,
             ...
             lineTop ;             // top pos of marker line rel to doc
        ...
        $.each(o, $.proxy(function (i, val)
           { 
             if (this._data.dnd[val])  {
               ...
               this._getContainer().removeClass("oj-invalid-drop") ;      // set cursor on tree div
               r = val;
               return false;
             }
        }, this));

        if (r === false)  {                           // not a valid drop position
          ctl.helper.children("ins")
                    .removeClass(OJT_DROP_OK)
                    .addClass(OJT_DROP_INVALID);
          vars.r.addClass(OJ_INVALID_DROP) ;
        }
        else {
          vars.r.removeClass(OJ_INVALID_DROP).addClass(OJ_VALID_DROP) ; 
        }
        ...
        {
            case "before":
                lineTop = this._data.dnd.off.top - 3 ;
            ...
            case "after":
                lineTop = this._data.dnd.off.top + this._data.core.li_height - 3 ;
                vars.m.css({ "left" : mLeft + "px",
                             "top"  : (lineTop - 2) + "px" 
                           }).show();
                if (vars.ml) {
            ...
            case "inside":
                vars.m.css({ "left" : mLeft + (this._isRtl? (isParent? -4 : 0) : 4) + "px",
                             "top" : (this._data.dnd.off.top + this._data.core.li_height/2 - 8) + "px" 
                            }).show();
                if (vars.ml)  {
                  vars.ml.hide();
                }
     ...
     /**
       *   A node's %lt;a&gt; link has been moused over, or the mouse is moving
       *   within it.
       *   @private
       */
     ...
     /**
       *  The mouse has left a node while dragging
       *   @private
       */
     _dnd_leave : function (e)
     {
        ...
        this._data.dnd.ctl.helper.children("ins")
                                 .removeClass(OJT_DROP_OK)
                                 .addClass(OJT_DROP_INVALID);
        this._getContainer().addClass("oj-invalid-drop") ;      // set cursor on tree div

        if (vars.o) {
          vars.o.removeClass(OJ_VALID_DROP).removeClass(OJ_INVALID_DROP) ;
        }
        if (vars.r) {
     ...
     /**
       *  Drop is complete - move the node. (_drag_stop will be called from the
       *                                     document mouseup handler later)
       *  @private
       */
     _dnd_finish : function (e)
      {
         ...
         if (dnd.foreign)   {
           ...
           this._dnd_prepare();
           this._moveNode(vars.o, vars.r, vars.last_pos, e[this.options["dnd"]["copy_modifier"] + "Key"]);
         }

         if (vars.o) {                          // remove classes from dragged node
           vars.o.removeClass(OJ_DRAG)
                 .removeClass(OJ_INVALID_DROP)
                 .removeClass(OJ_VALID_DROP) ;  
         }

         vars.o = false;
         vars.r = false;

         vars.m.hide();                 // hide the insert marker
         if (vars.ml) {
           vars.ml.hide();              // hide the insert marker line
         }
     },


     /**
       *  Mouse has gone down on a node's &lt;a&gt; link. Prepare for drag and drop.
       *  @private
       */
     _start_drag : function (obj, e)
     {
        ...
        var dt  = vars.o.length > 1 ? this._getString(TRANSKEY_MULTISEL) : this["getText"](vars.o),
        ...
        this._drag_start(e, { jstree : true,
                              obj    : vars.o
//                          }, "<ins class='oj-tree-icon'></ins>" + dt);        // we no longer use an icon
                            }, "<span>" + dt + "</span>");
     ...
     {  
         ...
         ctl.user_data = data;
         ctl.is_down   = true;
         ctl.helper    = $("<div class='oj-tree-title oj-drag' style='top:-2000px' />").html(html); //.fadeTo(10,0.25);   //@HTMLUpdateOK
     ...
     { 
        ...
        if (! ctl.is_drag)  {
          if (Math.abs(e.pageX - ctl.init_x) > 5 || Math.abs(e.pageY - ctl.init_y) > 5)  { 
            ctl.helper.appendTo("body");                                                     //@HTMLUpdateOK
            ctl.is_drag = true;
...
//                        Number() is for the closure compiler!!
        ...
        //  Position the dragged text relative to the mouse
        var horizDisp = ctl.helper_horiz ;

        if (this._isRtl && vars.o) {
          var $title = vars.o.find("span.oj-tree-title:first") ;
          if ($title && $title.length > 0) {
            horizDisp = - horizDisp - $title[0].offsetWidth ;
          }
        }
        ctl.helper.css({ left : (e.pageX + horizDisp) + "px",
                         top  : (e.pageY + ctl.helper_top) + "px" 
                       });
...
*/
     ...
     _initDnD : function()
     {
        ...
        this._getContainer()
            ...
            .bind("mouseup.ojtree", $.proxy(function (e)
               {
                  ...
                  {  // node should not be from the same tree
                      var tr = this._reference(e["currentTarget"]),     //TDO
                          dc;
                      if (tr["data"]["dnd"]["foreign"])  {
                        ...
                        if (dc === true || dc["inside"] === true || dc["before"] === true || dc["after"] === true)
                        {
                         ...
                         tr._moveNode(vars.o, tr._getContainer(), "last", e[tr.options["dnd"]["copy_modifier"] + "Key"]);
                      }
                  }
                 }, this))
            .bind("mouseleave.ojtree", $.proxy(function (e)
               {
                  ...
                  if (e.relatedTarget && $(e.relatedTarget).hasClass(OJT_DROP_MARKER_LINE))  {
                    return false; 
                  }
                  if (ctl.is_drag && ctl.user_data.jstree)  {
            ...
            {
              $(document)
             .delegate(s["drop_target"], "mouseenter.ojtree-" + this._getIndex(), $.proxy(function (e)
                         {
                           var dnd  = this._data.dnd,
                               vars = this._data.dnd.vars ;
             ...
             .delegate(s["drop_target"], "mouseup.ojtree-" + this._getIndex(), $.proxy(function (e)
                       {
                         var vars = this._data.dnd.vars ;
...
//     }
     ...
     _initData : function()
     {
        ...
        data.core  =  {
                        ...
                        toExpand       : false,
                        toLoad         : false,
                        prepared_move  : {},         // for the _moveNode function
       ...
       data.html.cloneMarkup        = false ;  // our clone of user's orig markup <li's>
        ...
        data.dnd.vars.o        = false ;     // node being dragged
        data.dnd.vars.r        = false ;     // reference node 
        ...
        data.dnd.ctl.helper_horiz = 15,      // horiz disp for dragged text div from mouse
        data.dnd.ctl.helper_top   = 5,       // top disp for dragged text div from mouse 
...
//   /**
//     * For easier debugging on tablet page via vpn.  Emits a _treetrace event
...
//
    ...
    /**
     * Removes the Tree from the DOM.  If the tree was constructed from original user &lt;ul&gt; markup
     * defined in the Tree's containing &lt;div&gt;, this markup is reinstated.
