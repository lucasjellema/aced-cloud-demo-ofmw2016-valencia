 ...
 * <code>&lt;ul id="menu" style="display:none">
...
oj.__registerWidget("oj.ojMenu", $['oj']['baseComponent'], {
    ...
    options: { // options is in externs.js, so no need for quotes
         ...
         * <code class="prettyprint">open()</code> method.  Those per-launch values can be further customized by a
         * <code class="prettyprint">beforeOpen</code> listener.
         * 
    ...
    _ComponentCreate: function() { // Override of protected base class method.  Method name needn't be quoted since is in externs.js.
        ...
        this._on({
            ...
            // Bug 19462004: Bad touch device behavior because the JQUI code relies on the above mouseenter handler to call _focus(), 
            ...
            "touchstart .oj-menu-item": function( event ) {
                // the touchHandled var ensures that this handler only runs for the originally touched
                // menu item, not for the parent menu items to which it bubbles.  Without it, get flakiness 
                // where sometimes a tap on a menu item inside the submenu doesn't "take".
                if ( this.touchHandled )
                    return;
                
                this.touchHandled = true;
                handleMouseEnterMenuItem(event);
            },
    ...
    */
                    ...
                    $.each(openPopupMenus , function(index, menu) {
                        ...
                        if (!$(event.target).closest(menu.element).length &&  // if event target is outside of menu element AND one of the following is true then close the menu.
                            (event.type === "keydown" || (event.type === "mousedown" && event.which === 3) ||  // 1. if it's a context-menu-launching event other than pressHold (see event.which on outer if)
                             ...
                             (menu._launcherClickShouldDismiss && ((event.type === "mousedown" && event.which !== 3) || event.type === "touchstart" ))))  // 3. If event is a (left/middle-mousedown or touchstart) on launcher and current menu is contextmenu (see bug 18745414)
                        {
    ...
    _destroy: function() { // Override of protected base class method.  Method name needn't be quoted since is in externs.js.
      ...
      // Destroy (sub)menus
        this.element
            ...
            .removeClass( "oj-component oj-menu-icons" )
    ...
    _setup: function() { // Private, not an override (not in base class).  Method name unquoted so will be safely optimized (renamed) by GCC as desired.
        ...
        submenus.filter( ":not(.oj-menu)" )
            ...
            .each(function() {
                ...
                item
                    ...
                    // @HTMLUpdateOK prepend trusted new DOM to menu item
                    .prepend( submenuIcon );
        ...
        var children = menus.children(); // <li>'s
...
//        }[ this.role ];
    ...
    // TODO: push up to Popup Fmwk, so all popups get this fix.
    // TODO: link to the JQ bug when I file it.
    /**
     ...
     *
     * To workaround the JQ bug, we copy the properties ourselves in this method.
     * 
     ...
     * @private
     */
    _normalizeEventForPosition: function( event ) {
    ...
    open: function( event, openOptions, submenuOpenOptions ) { // Public, not an override (not in base class), so use @expose with unquoted method name.
        ...
        this._normalizeEventForPosition(event); // see callee doc
