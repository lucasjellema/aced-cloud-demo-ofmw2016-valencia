...
{
    var item, contentContainer, context, inlineStyle, styleClass, renderer, content, textWrapper;

    item = document.createElement("li");
    $(item).uniqueId();
    parentElement.appendChild(item); // @HTMLUpdateOK
...
oj.TableDataSourceContentHandler.prototype.fetchRows = function(forceFetch)
{
    ...
    if (this.IsReady())
    {
        ...
        if (this.m_initFetch || forceFetch)
        {
            ...
            promise.then(function(value){ if (self.m_initFetch) { self._handleFetchedData(value); } }, 
...
oj.TableDataSourceContentHandler.prototype.handleModelAddEvent = function(event)
{
    ...
    data = event['data'];
    keys = event['keys'];
    if (data != null && keys != null && keys.length > 0 && data.length > 0 && keys.length == data.length)
    {
        index = this.m_root.childElementCount;
        for (i=0; i<data.length; i++)
        {
...
oj.TableDataSourceContentHandler.prototype._removeItem = function(elem)
{
    ...
    itemStyleClass = this.m_widget.getItemStyleClass();
    elem = $(elem);
    elem.children().wrapAll("<div class='"+itemStyleClass+"'></div>");
    elem.removeClass(itemStyleClass)
...
{
    var self = this;
    $(this.m_root).children("."+this.m_widget.getItemStyleClass()).each(function(index, elem)
    {
...
oj.TableDataSourceContentHandler.prototype.handleModelSyncEvent = function(event)
{
    ...
    // when paging control is used clear the list
    $(this.m_root).empty();
...
{
    var groupStyleClass, itemStyleClass, groupItemStyleClass, groupCollapseStyleClass, 
...
{
    var groupStyleClass, groupCollapseStyleClass, groupExpandStyleClass, itemStyleClass, itemElementStyleClass,
    ...
    for (i=0; i<items.length; i++)
    {
        ...
        {
            item.children("."+itemStyleClass).children().unwrap();      
...
{
    var itemStyleClass, itemElementStyleClass, groupStyleClass, groupItemStyleClass, groupCollapseStyleClass, collapseClass, 
...
{
    item.attr("role", this.IsHierarchical() ? "treeitem" : "option");
    if (this.m_widget._isSelectionEnabled() && this.isSelectable(context))
    {
        item.attr("aria-selected", false);
    }    
...
/** @lends oj._ojListView.prototype */
{
    ...
    notifyDetached: function() 
    {
        ...
        // For details see related button bug 18819266.
    ...
    getSubIdByNode: function(node)
    {
        ...
        if (node != null && $(node).hasClass(this.getExpandIconStyleClass()) || $(node).hasClass(this.getCollapseIconStyleClass()))
        {
            ...
            if (item != null && item.length > 0)
            {
                ...
                if (key != null)
                {
                    return {'subId': 'oj-listview-icon', 'key': key};
                }
            }           
        }
    ...
    { 
        ...
        {            
            ...
            if (this.m_contentHandler.IsHierarchical())
            {
                ...
                if (Array.isArray(expanded))
                {
                    ...
                    {
                        self.expandKey(value, false, false);
                    });
                }
            }
        }
        else if (options['selection'] != null)
        {            
        ...
        else if (options['selectionMode'] != null)
        {            
    ...
    {
        var expanded = item.attr("aria-expanded");        
        if (expanded == "true")
        {
    ...
    {
        var selection, i, index, clone, expanded, self, current;
        ...
        if (this._isSelectionEnabled())
        {
                    ...
                    if (selection[i] == context['key'])
                    {
                        ...
                        if (!this._isMultipleSelection())
                        {
                            if (selection.length > 1)
                            {
                                ...
                                this.SetOption("selection", [context['key']], {'_context': {originalEvent: null, internalSet: true}, 'changed': true});
                            }
                            break;
                        }
                    }
            ...
            else
            {
                ...
                if (index > -1)
                {
                    ...
                    this.SetOption("selection", clone, {'_context': {originalEvent: null, internalSet: true}, 'changed': true});
                }                
            }
        }
    ...
    HandleFocus: function(event)
    {
        ...
        {
            $(this.m_active['elem']).addClass("oj-focus");
        }
    },
    ...
    /**
     * Find the item element (recursively if needed)
     * @param {jQuery} elem
     * @return {jQuery|null}
     * @protected
     */
    ...
    {
        var top, height, container, containerScrollTop, containerHeight, scrollTop;
        ...
        scrollTop = Math.max(0, Math.min(top, Math.abs((top + height) - containerHeight)));
    ...
    _getItemsCache: function()
    {
        ...
        if (this.m_items == null)
        {
            ...
            this.m_items = this.element.find(selector).filter(function(index)
            {
                ...
                if (isGroup)
                {
                    return ($(this).parent().parent().attr("aria-expanded") != "false");
                } 
                return true;
            });
        }
    ...
    HighlightActive: function()
    {
        if (this.m_active != null)
        {   
            this._highlightElem(this.m_active['elem'], "oj-focus");
        }
    },
    ...
    _clearSelection: function(updateOption)
    {
        ...
        if (updateOption)
        {
            this.SetOption("selection", [], {'_context': {originalEvent: null, internalSet: true}, 'changed': true});
        }
    ...
    {
        var selection, items, startIndex, endIndex, from, to, i, item, key;
        ...
        this.SetOption("selection", selection, {'_context': {originalEvent: event, internalSet: true}, 'changed': true});
    },
     ...
     */
    expandKey: function(key, fireBefore, fireAfter)
    {
        var item = this.FindElementByKey(key);
        if (item != null)
        {
            this.ExpandItem($(item), null, false, key, fireBefore, fireAfter, fireBefore);    
        }
    },
     ...
     * @param {boolean} fireEvent trure if fire collapse event, false otherwise
     * @protected
     */
