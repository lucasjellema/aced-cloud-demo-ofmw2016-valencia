...
(function() {
    ...
    // var _heightAdjustmentApplied = false;  // initially set to false, once applied (in resizeBody(), we set it to true).
    var _heightAdjustmentApplied = true;  // initially set to false, once applied (in resizeBody(), we set it to true).
    ...
    */
	...
	_ComponentCreate : function ()
	{
	    ...
	    // this.ch - this.element.children();
	    // this.foo = this.element.children(".oj-dialog-footer");

	    // this.uiDialogFooter = this.element.find(".oj-dialog-footer");
	    // this.uiDialogFooter = this.element.find(".oj-dialog-footer").first();
	    ...
	    //
	    // Resize Listener - used for dialog resizes and browser resizes (for % heights)
	    //
	    this._hasResizeListener = false;

	    this._handleResizeFcn = this._resizeBody.bind(this);
	    if (this.uiDialog.length) {
		oj.DomUtils.addResizeListener(this.uiDialog[0], this._handleResizeFcn, _resizeDelay);
	...
	_destroy: function() {
      ...
      if (this.isOpen())
	    ...
	    // var isDraggable = this.uiDialog.is(":data(oj-draggable)");

	    var isDraggable = this.uiDialog.hasClass("oj-draggable");

	    if (this.uiDialog.draggable && isDraggable)
...
*/
	...
	open: function( event ) {
	    ...
	    if (this.options.resizeBehavior === "resizable") {
            ...
            psOptions[oj.PopupService.OPTION.POSITION] = this.options.position;
  ...
  _positionDescendents: function() {
	...
	_resizeBody: function() {
	    ...
	    //
	    // There are two cases when we need to resize the dialog body:
	    //   (a) if we are resizing the main dialog with the mouse
	    //   (b) if we are resizing the browser window and height is set as a percentage.
	    //
	    var hasPctHeight = false;
	    if (this.uiDialog.length && this.uiDialog[0].style.height)
		hasPctHeight = this.uiDialog[0].style.height.indexOf("%");

	    if (!this._isResizing) return;
	    if (!hasPctHeight) return;

	    var bodyResized = false;
...
/*
	    ...
	    if (heightValue == "" || heightValue == "auto" || hasPctHeight) {

		var scrollHeight = this.element[0].scrollHeight;
		var offsetHeight = this.element[0].offsetHeight;

		// has_scrollbar when scrollheight > offsetheight

		//
		// Address a chrome bug that surfaces when there are vertical margins that are fractional.
		// (We get a scrollbar when we shouldn't).
		// (This code block is likely not needed, since we simplified the use of resize listeners
		//
		if (offsetHeight > 0 && scrollHeight >= offsetHeight) {
		    if (!_heightAdjustmentApplied) {

			if (window.chrome) {
			    var fixDelta = scrollHeight - offsetHeight;
			    if (fixDelta > 0) bodyHeight += fixDelta;
			    bodyHeight += 1;
			    this.element.css({height: bodyHeight});
			    bodyResized = true;
			}

			_heightAdjustmentApplied = true;
...
*/
	...
	_getBodyHeight: function() {
	    ...
	    // Note - do not try to use uiDialog.outerHeight() - this will cause an infinite resize
	    ...
	    // This calculation seems to work better for chrome, but causes infinite height resizing issues in firefox.
	    // var bodyHeight =  Math.ceil(this.uiDialog.height()) - headerHeight - footerHeight;
	...
	_size: function() {
	    ...
	    // It is also needed to stretch the .oj_dialog_body height
	    // (for the case when the dialog height is set larger than is required)
	    //
	    if (heightValue != "auto" && heightValue != "") {
   ...
   * <li><b>oj-dialog-close-icon</b>: dialog header-close icon </li>
