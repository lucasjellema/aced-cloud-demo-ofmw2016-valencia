 ...
 */
  oj.Events.EventType = {
         ...
         * The event passes these arguments to the handler:<br>
         * <ul>
         * <b>model</b>: the model on which the save or property set is invalid <br>
         * <b>validationError</b>: the model's validationError property<br>
 ...
 *                  <b>parse</b>: a user callback function to allow parsing of the JSON collection after it's returned from the data service<br>
 ...
 *                  if deferred is specified and true, at will return a Promise object which will resolve passing the model at the given index
 * @export
 */
...
oj.Collection.prototype._setPagingReturnValues = function(response, options, data, fillIn) {
    ...
    if (!fillIn) {
        ...
        var totalResultsReturned = parseInt(oj.Collection._getProp(customObj, response, 'totalResults'),10);
...
oj.Collection.prototype._adjustTotalResults = function(totalResultsReturned, hasMore, offset, lastFetchCount, dataLength) {
    // Fix for bug 21239986: if hasMore is false, and totalResults wasn't set by the server, we should set it to the last 
 ...
 * @param {Object=} options Options to control save (see [save]{@link oj.Model#save}).  Plus:<p>
 *                          <b>deferred</b>: if true, return a promise as though this collection were virtual whether it is or not<br>
...
oj.Collection.prototype._setInternal = function(models, parse, options, deferred) {
    ...
    modelList = $.isArray(models) ? models : [models];

    if (deferred) {
        return this._addPromise(function() {
            return this._deferredSet(modelList, this._getModels(), options, remove, add, merge, parse);
        });
    }    
...
oj.Collection._getQueryString = function(q) {
    ...
    var queries = $.isArray(q) ? q : [q];
    var str = "", query, exp, i, prop;
    for (i = 0; i < queries.length; i++) {
        query = queries[i];
        for (prop in query) {
            if (query.hasOwnProperty(prop)) {
                var val = $.isArray(query[prop]) ? query[prop] : [query[prop]];
...
oj.Model.prototype.set = function (property, value, options) {
    var opts = options || {}, ignoreLastArg = false, prop, i, valid = true;
...
oj.Model.prototype._hasAttrs = function(attrs) {
    ...
    for (prop in attrs) {
        if (attrs.hasOwnProperty(prop)) {
            ...
            var val = $.isArray(attrs[prop]) ? attrs[prop] : [attrs[prop]];
...
oj.Model.prototype.toJSON = function() {
    ...
    for (prop in this.attributes) {
        if (this.attributes.hasOwnProperty(prop)) {
            if ($.isArray(this.attributes[prop])) {
