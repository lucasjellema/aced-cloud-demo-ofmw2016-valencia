 ...
 */
DvtGaugeRenderer.renderLabel = function(gauge, container, bounds, color, halign, valign, bMouseTransparent) {
  var options = gauge.getOptions();
  var rendered = false;
  ...
  if (options['metricLabel']['rendered'] == 'on') {
    ...
    if (valign == 'top') {
      ...
      label.setY(bounds.y + bounds.h / 2 - DvtTextUtils.getTextHeight(label) / 2);
    }
 ...
 * @return {boolean} Return true if the title is rendered
 */
DvtGaugeRenderer.renderTitle = function(gauge, container, bounds, color, bMouseTransparent) {
  var options = gauge.getOptions();
  var rendered = false;
  ...
  if (DvtGaugeStyleUtils.hasTitle(options)) {
    ...
    if (rendered) {
      ...
      else if (title.isTruncated()) // Show tooltip for truncated text
        gauge.__getEventManager().associate(title, new DvtSimpleObjPeer(titleString));
    }

    var textDim = title.getDimensions();
    title.setY(bounds.y + bounds.h / 2 - textDim.h / 2);
...
DvtLedGaugeRenderer.render = function(gauge, container, width, height) {
  if (DvtGaugeDataUtils.hasData(gauge)) {
    ...
    if (DvtGaugeStyleUtils.hasTitle(options)) {
      ...
      if (options['metricLabel']['rendered'] == 'on') {
        titleBounds.y = titleBounds.y + titleBounds.h * 2 / 3;
        titleBounds.h = titleBounds.h * 1 / 3;
      }
      ...
      if (!DvtAgent.isHighContrast() && options['title']['style'].getStyle('color') != null) {
        bTitleRendered = DvtGaugeRenderer.renderTitle(gauge, container, titleBounds, null, true);
      }
      else {
        bTitleRendered = DvtGaugeRenderer.renderTitle(gauge, container, titleBounds, labelColor, true);
      }
    }
    ...
    if (options['metricLabel']['rendered'] == 'on') {
      ...
      if (bTitleRendered)
        labelBounds.h = labelBounds.h * 2 / 3;

      // Use the specified label color unless in high contrast mode
      if (!DvtAgent.isHighContrast() && options['metricLabel']['style'].getStyle('color') != null) {
        DvtGaugeRenderer.renderLabel(gauge, container, labelBounds, null, 'center', 'middle', true);
      }
      else {
        DvtGaugeRenderer.renderLabel(gauge, container, labelBounds, labelColor, 'center', 'middle', true);
      }
    }
  }
...
DvtStatusMeterGaugeRenderer._renderCircularGauge = function(gauge, container, bounds) {
  ...
  if (DvtGaugeStyleUtils.hasTitle(options)) {
    ...
    if (options['metricLabel']['rendered'] == 'on') {
      titleSpace.y = titleSpace.y + titleSpace.h * 2 / 3;
      titleSpace.h = titleSpace.h / 3;
    }
    bTitleRendered = DvtGaugeRenderer.renderTitle(gauge, container, titleSpace, null, true);
    if (bTitleRendered && options['metricLabel']['rendered'] == 'on')
      labelBounds.h = labelBounds.h * 2 / 3;
  }
  DvtGaugeRenderer.renderLabel(gauge, container, labelBounds, null, 'center', 'middle', true);
...
DvtStatusMeterGaugeRenderer._renderLabelOutsidePlotArea = function(gauge, container, bounds) {
  ...
  // Allocate space for the horizontal label
    ...
    label.setY(bounds.y + bounds.h / 2 - maxLabelDims.h / 2);
    label.alignRight();
...
DvtStatusMeterGaugeRenderer._renderLabelInsidePlotArea = function(gauge, container, bounds, color, labelPosition, indicator, plotArea, repeatedTry) {
  ...
  var labelRendered = DvtGaugeRenderer.renderLabel(gauge, container, labelSpace, labelColor, hAlignment, vAlignment, true);
...
DvtStatusMeterGaugeRenderer._renderTitle = function(gauge, container, bounds, labelPosition) {
  ...
  if (options['title']['position'] == 'center' || (options['title']['position'] == 'auto' && isVert)) {
    title.setY(bounds.y + bounds.h / 2 - titleDim.h / 2);
    title.setX(bounds.x + bounds.w / 2);
    title.alignCenter();
  }
  else {
    ...
    else if (!isVert && !isRTL)
    {
      ...
      title.setY(bounds.y + bounds.h - titleDim.h - titleSpace.w / 2);
      title.setX(bounds.x + bounds.w / 2);
      title.alignCenter();
    }
  }
