...
DvtDataGrid.prototype.setDefaultDimensions = function()
{
    ...
    // Not using offsetWidth/Height due to bug 20326588
...
DvtDataGrid.prototype.resetInternal = function()
{
    ...
    this.m_avgColHeaderWidth = undefined;    
...
DvtDataGrid.prototype.buildColumnHeaders = function(headerRoot, headerSet, start, totalCount)
{
    ...
    if (headerCount > 0)
    {
        avgWidth = totalColumnWidth / headerCount;
...
DvtDataGrid.prototype.buildLevelHeaders = function(fragment, index, level, left, top, isAppend, insert, renderer, headerSet, axis, className, totalLevels)
{
    var getLevelDimension, getHeaderDimension, levelDimension, headerDimension, dimensionToAdjust, dimensionToAdjustValue, dimensionSecond, dimensionSecondValue,
            ...
            headerContent, headerDepth, inlineStyle, styleClass, nextIndex, totalLevelDimensionValue, returnObj, content, textWrapper, sortIcon;
    ...
    if (patchBefore && index === end + 1)
    {
        ...
        for (i = 0; i < headerExtent; i)
        {
            if (axis === 'column')
            {
                returnVal = this.buildLevelHeaders(groupingContainer, index + i, level + 1, dimensionToAdjustValue, dimensionSecondValue + levelDimensionValue, isAppend, insert, renderer, headerSet, axis, className, totalLevels);
            }
            else
            {
                returnVal = this.buildLevelHeaders(groupingContainer, index + i, level + 1, dimensionSecondValue + levelDimensionValue, dimensionToAdjustValue, isAppend, insert, renderer, headerSet, axis, className, totalLevels);
            }
    ...
    else
    {
        ...
        levelDimensionValue = getLevelDimension(level, header);
        this.setElementDir(header, dimensionToAdjustValue, dimensionToAdjust);
        ...
        if (level != totalLevels - 1)
        {
            ...
            groupingContainer.appendChild(header);
        ...
        else
        {
            for (i = 0; i < headerExtent; i++)
            {
                ...
                if (axis === 'column')
                {
                    returnVal = this.buildLevelHeaders(groupingContainer, nextIndex, level + 1, dimensionToAdjustValue, dimensionSecondValue + levelDimensionValue, isAppend, insert, renderer, headerSet, axis, className, totalLevels);
                }
                else
                {
                    returnVal = this.buildLevelHeaders(groupingContainer, nextIndex, level + 1, dimensionSecondValue + levelDimensionValue, dimensionToAdjustValue, isAppend, insert, renderer, headerSet, axis, className, totalLevels);
                }
        ...
        if (isAppend)
        {
            ...
            {
                fragment.appendChild(groupingContainer);
            }
            else
            {
...
{
    var headerCount, scroller, resizer, renderer, isAppend, top, totalRowHeight, className, 
    ...
    {
        scroller.insertBefore(fragment, resizer['nextSibling']); //@HTMLUpdateOK
...
DvtDataGrid.prototype._getRowHeaderContainer = function(index, level, currentLevel, rowHeaders)
{
    ...
    // loop over all headers skipping resizer and firstChild of groups
    for (i = 1; i < rowHeaders.length; i++)
    {
        ...
        {
            // if we are on the right level we know the header container is this one
            if (level === currentLevel)
            {
                return rowHeaders[i];
            }
            //check one level deeper
            return this._getRowHeaderContainer(index, level, currentLevel + 1, rowHeaders[i]['childNodes']);
        }
    }
    return null;
};
...
DvtDataGrid.prototype._getRowHeaderByIndex = function(index, level)
{
    ...
    if (this.m_rowHeaderLevelCount === 1)
    {
        ...
        relativeIndex = index - this.m_startRowHeader + 1;
...
DvtDataGrid.prototype._getColumnHeaderContainer = function(index, level, currentLevel, columnHeaders)
{
    ...
    if (columnHeaders == null)
    {
        ...
        // if we are on the scroller children there is no resizer or first header so start at the first header in the list
        i = 0;
    }
    else
    {
    ...
    for (i; i < columnHeaders.length; i++)
    {
        ...
        if (index >= headerIndex && index < headerIndex + headerExtent)
        {
            ...
            return this._getColumnHeaderContainer(index, level, currentLevel + 1, columnHeaders[i]['childNodes']);
        }
    }
    return null;
};
 ...
 * @param {Array.<Object>} cellRange - [rowRange, columnRange] - [{"axis":,"start":,"count":},{"axis":,"start":,"count":,"databody":,"scroller":}]
 * @param {boolean} rowInsert - if this is triggered by a row insert event
 * @param {Object=} headerSet - a HeaderSet object which encapsulates the result set of row headers
 * @param {boolean=} obj
 * @protected
 */
DvtDataGrid.prototype.handleCellsFetchSuccess = function(cellSet, cellRange, rowInsert, headerSet, obj)
{
    var totalRowCount, totalColumnCount, defaultHeight, rowRange, rowStart, rowCount,
            ...
            databody, top, referenceRow, scroller, inner, databodyContent, resizer, referenceRowHeader,
            isAppend, fragment, headerFragment, totalRowHeight, i, j, avgHeight, avgWidth, duration, self,         
            rows, totalColumnWidth, prev, addResult, topHotspot, bottomHotspot, rowheaders, rowheadersContent,
            rowHeaderCount; 
    ...
    if(headerSet) 
    {
        rowHeaderCount = headerSet.getCount("row");
    }
    ...
    if (rowRangeNeedsUpdate || rowInsert)
    {
        ...
        if(headerSet)
        {
            rowheadersContent = rowheaders['firstChild'];
            referenceRowHeader = rowheadersContent['childNodes'][rowStart-this.m_startRow+1];           
            headerFragment = this.buildRowHeaders(null, headerSet, 0, -1, false, true); 
        }   
        ...
        if(headerSet)
        {
            ...
            {
                headerFragment.childNodes[j].style.top = fragment.childNodes[j].style.top;            
            }
        }
        
        if (isAppend)
        {
            if(obj && obj == true && this.m_utils.supportsTransitions()) 
            {
                this._insertRowsWithAnimation(databodyContent, fragment, referenceRow, rowStart, avgHeight, rowheadersContent, headerFragment, referenceRowHeader, duration, 'ease-out', isAppend);
            }
            else
            {
            ...
            if(headerSet) 
            {
                rowheadersContent.appendChild(headerFragment); 
                this.m_endRowHeader = rowStart + rowHeaderCount - 1; 
                this.m_endRowHeaderPixel = this.m_endRowHeaderPixel + totalRowHeight; 
            }
        }
        else if (rowInsert)
        {
            if(obj && obj == true && this.m_utils.supportsTransitions()) 
            {
                this._insertRowsWithAnimation(databodyContent, fragment, referenceRow, rowStart, avgHeight, rowheadersContent, headerFragment, referenceRowHeader, duration, 'ease-out');
            }
            ...
            else
            {
                ...
                if(headerSet)
                {
                    rowheadersContent.insertBefore(headerFragment, referenceRowHeader); //@HTMLUpdateOK
                }                  
            }
            ...
            if (headerSet)
            {
                if (rowStart < this.m_startRowHeader)
                {
                    // added in the middle
                    this.m_startRowHeader = rowStart;
                    this.m_startRowHeaderPixel = Math.max(0, this.m_startRowHeaderPixel - totalRowHeight);
                }                
                //update the endRow and endRowPixel no matter where we insert            
                this.m_endRowHeader = this.m_endRowHeader + rowCount;
                this.m_endRowHeaderPixel = this.m_endRowHeaderPixel + totalRowHeight;                      
            }       
            
            if(!obj || !this.m_utils.supportsTransitions()) 
            {            
                this.pushRowsDown(referenceRow, totalRowHeight);
                if(headerSet)
                {
                    this.pushRowsDown(referenceRowHeader, totalRowHeight); 
                }
            }
            ...
            else 
            {
                if (totalRowCount != -1 && !this._isHighWatermarkScrolling()) 
                {
                    ...
                    if(headerSet)
                    {
                        this.setElementHeight(this.m_rowHeader, totalRowCount * avgHeight);
                    }                       
                }
                else
                {
                    ...
                    if(headerSet)
                    {
                        this.setElementHeight(this.m_rowHeader, this.m_endRowPixel);
                    }                    
                }            
                this.pushRowsDownWithAnimation(referenceRow, totalRowHeight, duration+30, 0, 'ease-out');
                this.pushRowsDownWithAnimation(referenceRowHeader, totalRowHeight, duration+30, 0, 'ease-out');                
            }            
        }
        else
        {            
            databodyContent.insertBefore(fragment, resizer['nextSibling']); //@HTMLUpdateOK
 ...
 * @param {Element} rowheadersContent
 * @param {Element} headerFragment
 * @param {Element} referenceRowHeader
 * @param {number} duration animation duration.
 * @param {string} timing easy function name.
 * @param {Boolean=} isAppend.
 * @private
 */
DvtDataGrid.prototype._insertRowsWithAnimation = function(databodyContent, fragment, referenceRow, rowStart, avgHeight, rowheadersContent, headerFragment, referenceRowHeader, duration, timing, isAppend)
{
    var i, j, jk, shift, tops, beforeCount, startRowIdx, self,
        transition_duration, z_index, transition_timing, transform, rowHeader, isRowHeader = false;
    ...
    if(isRowHeader)
    {
        this.changeStyleProperty(rowheadersContent['childNodes'][startRowIdx], this.getCssSupport('z-index'), 1000);
    }    
    ...
    //move all new rows under clicked row
    for(jk=0; jk < fragment.childNodes.length; jk++)
    {
        this.addTransformMoveStyle(fragment.childNodes[jk], 0, 0, 'linear', 0, '-'+tops[jk], 0);
        if(isRowHeader)
        {
            this.addTransformMoveStyle(headerFragment.childNodes[jk], 0, 0, 'linear', 0, '-'+tops[jk], 0);
        }         
    }
    ...
    {    
        ...
        {
            self.addTransformMoveStyle(databodyContent.childNodes[j], duration + "ms", 0, timing, 0, 0, 0);
            if(isRowHeader)
            {
                self.addTransformMoveStyle(rowheadersContent.childNodes[j], duration + "ms", 0, timing, 0, 0, 0);
            }             
        }            
        ...
        {        
            ...
            for (i = 1; i < databodyContent.childElementCount; i++)
            {
                if(databodyContent.children[i].style[self.getCssSupport('transform')])
                {
                    ...
                    self.removeTransformMoveStyle(databodyContent.childNodes[i]);
            ...
            if(isRowHeader)
            {
                for (i = 1; i < rowheadersContent.childElementCount; i++)
                {
                    if(rowheadersContent.children[i].style[self.getCssSupport('transform')])
                    {
                        self.changeStyleProperty(rowheadersContent['childNodes'][startRowIdx], z_index, 0, "remove");
                        self.removeTransformMoveStyle(rowheadersContent.childNodes[i]);
                    }
                }            
            }
            ...
            if(!isAppend)
            {
                ...
                if(isRowHeader)
                {            
                    self.pushRowsDown(referenceRowHeader, shift);
                }
            }
        }, false);
    }, 0);
 ...
 * @private
 */
DvtDataGrid.prototype.pushRowsDownWithAnimation = function(row, adjustment, duration, delay, easing) // expand animation
{
    while (row)
    {
        this.addTransformMoveStyle(row, duration + "ms", delay + "ms", easing, 0, adjustment, 0);
        row = row['nextSibling'];
    }
};
...
DvtDataGrid.prototype.getFocusableElementsInNode = function(node, skipTabIndexCheck)
{
    ...
    if (document.evaluate)
    {
        ...
        while (elem)
        {
            if (!elem.disabled && (skipTabIndexCheck || !elem.tabIndex || elem.tabIndex > 0) && this.isElementVisible(elem, node))
            {
    ...
    else
    {
        ...
        {
            elem = nodes[i];
            if (elem.tagName.match(inputRegExp) && !elem.disabled && (skipTabIndexCheck || !elem.tabIndex || elem.tabIndex > 0) && this.isElementVisible(elem, node))
            {
                inputElems.push(elem);
            }
        }
...
DvtDataGrid.prototype._enableAllFocusableElements = function(cell)
{
    ...
    focusElems = this.getFocusableElementsInNode(cell, true);
    for (i=0; i<focusElems.length; i++)
    {
        if (focusElems[i].getAttribute('tabIndex') == -1)
        {
...
DvtDataGrid.prototype.scrollDelta = function(deltaX, deltaY)
{
    ...
    else
    {
        ...
        scrollLeft = Math.max(0, Math.min(this.m_scrollWidth, scrollLeft));
        scrollTop = Math.max(0, Math.min(this.m_scrollHeight, scrollTop));
...
{
    var rowHeaderContent, resizer, resizerHeight, rowThreshold, returnVal;
    ...
    returnVal = this.removeRowHeadersFromTopOfContainer(rowHeaderContent, resizer, resizerHeight, rowThreshold);
    
    this.m_startRowHeaderPixel += returnVal['heightChange'];
    this.m_startRowHeader += returnVal['extentChange'];
    
    this.setElementHeight(resizer, resizerHeight + returnVal['heightChange']);
};
 ...
 */
DvtDataGrid.prototype.removeRowHeadersFromTopOfContainer = function(rowHeadersContainer, resizer, resizerHeight, rowThreshold)
{
...
DvtDataGrid.prototype.handleContextMenuGesture = function(event, eventType, callback)
{
    ...
    cell = this.findCell(event['originalEvent'].target);
    if (eventType === 'touch' && cell != null)
    {
        cell = this.findCell(event['originalEvent'].target);
        index = {"row": this.getRowIndex(cell['parentNode']), "column": this.getCellIndex(cell)};
        ...
        {
            this._openContextMenu(event, eventType, callback)
        }
        else
        {
            // we have a new active or selection so store the callback to be called after focus
            this.m_contextMenuCallback = this._openContextMenu.bind(this, event, eventType, callback);                
            if (this._isSelectionEnabled())
            {                   
...
/**
 * Function to open the context menu by passing the capabilites and launcher to the wrapper
 * @param {Event} event the original context menu event
 * @param {string} eventType the event type touch/keyboard/mouse
 * @param {function} callback the wrapper method to callback to to open the context menu
 * @returns {Object|undefined} {capabilites:, launcher:}
 */
DvtDataGrid.prototype._openContextMenu = function(event, eventType, callback) 
{
    var element, capabilities, launcher, target;
    ...
    if (element != null)
    {
        ...
        if (this.isMultipleSelection() && this._isContainSelection(this.getCellIndex(element)))
        {
            ...
            if (this.m_active != null)
            {
                launcher = this.getElementsInRange(this.createRange(this.m_active))[0];
...
{
    var capabilities, rowHeader, columnHeader, resizable, sortable, sameColumn = true, sameRow = true, disable, enable;
    disable = 'disable';
    enable = 'enable';
...
{
    var capabilities, resizable, sortable, sameColumn = true, sameRow = true, disable, enable;
    disable = 'disable';
    enable = 'enable';
    capabilities = {'resize': disable, 'resizeWidth': disable, 'resizeHeight': disable,
        'sortRow': disable, 'sortCol': disable, 'cut': disable, 'paste': disable};    
...
DvtDataGrid.prototype.handleContextMenuReturn = function(event, id, value)
{
    ...
    if (this.m_active != null)
    {
        target = this.getElementsInRange(this.createRange(this.m_active))[0];        
    }
    else
    {
...
{
    var duration, diffX, diffY, momentumX, momentumY, transitionDuration, rtl;
    ...
    if (this.m_touchActive && !event.defaultPrevented)
    {
        ...
        else
        {
            ...
            // swipe case
            if (duration < DvtDataGrid.MAX_SWIPE_DURATION)
            {
                event.preventDefault();
...
{
    var touchEnd, touchLength, tapMax = DvtDataGrid.HEADER_TAP_SHORT_HOLD_DURATION,
            momentumX, momentumY, transitionDuration, duration, rtl, diffX, diffY;
    ...
    {        
        // if a short tap sort
        if (this.m_currentX == this.m_startX && this.m_currentY == this.m_startY && touchLength < tapMax)
        {
            if (this._isDOMElementSortable(event.target))
            {
                event.preventDefault();                
        ...
        else if (this.m_isResizing && this.isResizeEnabled())
        {
            this.handleResizeMouseUp(event);
            if (this.m_currentX != this.m_startX && this.m_currentY != this.m_startY)
            {
...
DvtDataGrid.prototype.getElementHeight = function(elem)
{
    ...
    else
    {
        if (!document.body.contains(elem))
        {
            ...
            // Not using offsetHeight due to bug 20326588
            height = Math.round(elem.getBoundingClientRect()['height']);
...
DvtDataGrid.prototype.getElementWidth = function(elem)
{
    ...
    else
    {
        if (!document.body.contains(elem))
        {
            elem['style']['visibility'] = "hidden";
            document.body.appendChild(elem);
            // Not using offsetWidth due to bug 20326588            
            width = Math.round(elem.getBoundingClientRect()['width']);
            document.body.removeChild(elem);
            elem['style']['visibility'] = ""; 
        }   
        else
...
{
    var operation, keys, cellSet, headerSet, indexes, source, indices, fragment, silent; 
    ...
    if (operation === 'insert')
    {
        ...
        if (cellSet != null)
        {
            ...
            {
                this._handleModelInsertRangeEvent(cellSet, headerSet, true);
            }            
            else
            {
    ...
    for (i=0; i<keys.length; i++)
    {
        ...
        while (j--) 
        {
            ...
            if (startRowKey == rowKey)
            {
                ...
                newRowKey = this._getKey(this.m_databody['firstChild']['childNodes'][range['startIndex']['row'] + 1 - this.m_startRow + 1]);
            ...
            else if (endRowKey == rowKey)
            {
                ...
                // get the key of the next row, which will become the new start key
                newRowKey = this._getKey(this.m_databody['firstChild']['childNodes'][range['endIndex']['row'] - 1 - this.m_startRow + 1]);
 ...
 * @param {Objec=} headerSet - a HeaderSet object which encapsulates the result set of row headers
 * @param {boolean=} obj
 */
DvtDataGrid.prototype._handleCellInsertsFetchSuccess = function(cellSet, cellRanges, headerSet, obj)
{
    var rowStart;
    ...
    // insert the row
    this.handleCellsFetchSuccess(cellSet, cellRanges, this.m_endRow >= cellRanges[0]['start'], headerSet, obj); 
 ...
 * @private
 */
DvtDataGrid.prototype._handleModelInsertRangeEvent = function(cellSet, headerSet, obj)
{
    var rowRange, columnRange;
    ...
    // insert the rows
    this._handleCellInsertsFetchSuccess(cellSet, [rowRange, columnRange], headerSet, obj);
};
...
DvtDataGrid.prototype._handleHeaderUpdatesFetchSuccess = function(headerSet, headerRange)
{
    ...
    row = rowHeaderContent.childNodes[rowIndex+1];  // +1 since first one is resizer
...
{
    var key, i, j, k, rowKey, row, totalHeight, height, referenceRow, referenceRowHeader, databodyContent,
        rowHeader, duration, clickedRow, start, self, rowHeaderContent, clickedRowHeader,
        transition_duration, transition_delay, transition_timing, transform, rowHdr, parent, headerParent;
    ...
    parent = self._findRowByKey(keys[0]['row'])['parentNode'];
    
    //all inherited animated rows should be hidden under primary collapsed one
    this.changeStyleProperty(parent, self.getCssSupport('z-index'), 10);
    if(self._findRowHeaderByKey(keys[0]['row']))
    {
        headerParent = self._findRowHeaderByKey(keys[0]['row'])['parentNode'];
        this.changeStyleProperty(headerParent, self.getCssSupport('z-index'), 10);
    }
    ...
    for (i=keys.length-1; i>=0; i--)
    {
        ...
        if (key['row'])
        {
            ...
            if (row != null)
            {
                ...
                //add animation CSS rules to each row's style
                self.addTransformMoveStyle(row, duration + "ms", 0, 'ease-out', 0, '-'+row['offsetTop'], 0);
                ...
                if(i == keys.length-1)
                {
                    ...
                    {                              
                        ...
                        {                            
                            ...
                            for (k = 1; k < databodyContent.childElementCount; k++)
                            {
                                if(self._getKey(clickedRow))
                                {                        
                                    if(self._getKey(clickedRow) == self._getKey(databodyContent.childNodes[k]))
                                    {
                                        start = k+1;
                                    }
                                }
                                ...
                                {                                    
                                    self.removeTransformMoveStyle(databodyContent.childNodes[k]);
                                }
                                //and assign correct top values instead
                                if(start > 0)
                                {
                                    //TODO: Why can't we just keep track of top values by adding height instead of assuming uniformity
                                    databodyContent.childNodes[k].style.top = clickedRow.offsetTop + height*(k-start+1) + 'px';
                                }                               
                            }
                            self.changeStyleProperty(parent, self.getCssSupport('z-index'), 0, "remove");
                            if(headerParent)
                            {
                                self.changeStyleProperty(headerParent, self.getCssSupport('z-index'), 0, "remove");
                            }
                        },0);
                        self.resizeGrid();
                        // check viewport to see if we need to fetch because of deleted row causing empty spaces
            ...
            if (rowHeader != null)
            {
                rowHeaderContent = self.m_rowHeader['firstChild'];
                referenceRowHeader = rowHeader['nextSibling'];
                self.changeStyleProperty(rowHeader, self.getCssSupport('z-index'), "-1"+(i+1));
                self.addTransformMoveStyle(rowHeader, duration + "ms", 0, 'ease-out', 0, '-'+row['offsetTop'], 0);
                                
                if(i == keys.length-1)
                {
                    rowHeader.addEventListener('transitionend', function() 
                    {
                        for (var jj=0; jj<keys.length; jj++)
                        {
                            if (keys[jj]['row'])
                            {
                                rowHdr = self._findRowHeaderByKey(keys[jj]['row']);
                                if(rowHdr)
                                {
                                    self._remove(rowHdr);
                        ...
                        {
                            start = -1;
                            for (var kk = 1; kk < rowHeaderContent.childElementCount; kk++)
                            {
                                if(self._getKey(clickedRow))
                                {                        
                                    if(self._getKey(clickedRow) == self._getKey(rowHeaderContent.childNodes[kk]))
                                    {
                                        start = kk+1;
                                    }
                                }
                                //clean all animation (transition) parameters for each animated row if required
                                self.changeStyleProperty(rowHeaderContent.childNodes[kk], self.getCssSupport('z-index'), 0, "remove");
                                if(rowHeaderContent.children[kk].style[transform])
                                {                                    
                                    self.removeTransformMoveStyle(rowHeaderContent.childNodes[kk]);
                                }
                                //and assign correct top values instead
                                if(start > 0)
                                {
                                    rowHeaderContent.childNodes[kk].style.top = clickedRow.offsetTop + height*(kk-start+1) + 'px';
                                }                               
                            }
                        },0);               
                        this.removeEventListener('transitionend',arguments.callee,false);
    ...
    self.pushRowsUpWithAnimation(self, referenceRow, databodyContent, height*keys.length, duration, 0, 'linear', false);
    if(referenceRowHeader) 
    {
        self.pushRowsUpWithAnimation(self, referenceRowHeader, rowHeaderContent, height*keys.length, duration, 0, 'linear', true);
    }
};
 ...
 * @private
 */
DvtDataGrid.prototype.pushRowsUpWithAnimation = function(self, row, content, adjustment, duration, delay, easing, isRowHeader) 
{
    var i, scrollerContent, contentHeight;
    
    contentHeight = 0;
    while (row)
    {        
        //add transition animation rules to the row's CSS style
        self.addTransformMoveStyle(row, duration + "ms", delay + "ms", easing, 0, "-"+adjustment, 0); 
        //detect the last row to add transition end event listener
        if(!row['nextSibling'])
        {
            // adjust the databody height
            scrollerContent = self.m_scroller['firstChild'];
            for(i=1; i<content.childElementCount; i++)
            {
                contentHeight += content.childNodes[i].clientHeight;
            }
            self.setElementHeight(content, contentHeight);
            self.setElementHeight(scrollerContent, contentHeight);
...
DvtDataGrid.prototype._setActiveCallback = function(event, callback, keys, indexes)
{
    ...
    if ((this.m_active == null) || (
            ...
            indexes['column'] != this.m_active['columnIndex'] || 
            indexes['row'] != this.m_active['rowIndex']))
    {
...
DvtDataGrid.prototype.highlightActive = function(focus)
{
    ...
    if (this.m_contextMenuCallback != null)
    {
        this.m_contextMenuCallback.call();
        this.m_contextMenuCallback = null;
    }        
};
...
DvtDataGrid.prototype.getHeaderFromCell = function(cell, axis)
{
    ...
    if (axis === 'row') 
    {
        ...
        {
            row = cell['parentNode'];
            rowIndex = this.findIndexOf(row) - 1 + this.m_startRow;
            if (rowIndex > -1)
            {             
...
{
    var keyCode, target, cell, focusElems;
    ...
    {        
        if (this.isActionableMode())
        {
            ...
            if (!this._isContainFocusableElement(cell))
            {
                this.setActionableMode(false);
                this._disableAllFocusableElements(cell);
            }
    ...
    if (this.isActionableMode())
    {
        ...
        if (keyCode == DvtDataGrid.ESC_KEY)
        {
            ...
            // disable all focusable elements in the cell again
            this._disableAllFocusableElements(cell);
        ...
        else if (keyCode == DvtDataGrid.TAB_KEY)
        {
            ...
            focusElems = this.getFocusableElementsInNode(cell);
            if (focusElems.length > 0 && target == focusElems[focusElems.length-1])
            {
    ...
    else
    {
        ...
        if (keyCode == DvtDataGrid.F2_KEY || keyCode == DvtDataGrid.ENTER_KEY)
        {
            ...
            if (this._setFocusToFirstFocusableElement(cell))
            {
                this.setActionableMode(true);

                // enable all focusable elements
                this._enableAllFocusableElements(cell);
            }
        }
    }
...
{
    var axis, index, elem, keyCode, processed = false, level, ctrlKey;
    ...
    if (this.isArrowKey(keyCode) && !ctrlKey)
    {
    ...
    else if (keyCode == DvtDataGrid.PAGEUP_KEY)
    {
        if (this.m_activeHeader['axis'] === 'row')
        {
            ...
            elem = this._getRowHeaderByIndex(this.m_startRowHeader, level);
            this._setActiveHeader(this.m_startRowHeader, this._getKey(elem), axis, level);
            processed = true;
        }
    }
    else if (keyCode == DvtDataGrid.PAGEDOWN_KEY)
    {
        if (this.m_activeHeader['axis'] === 'row')
        {
            ...
            elem = this._getRowHeaderByIndex(this.m_endRowHeader, level);
            this._setActiveHeader(this.m_endRowHeader, this._getKey(elem), axis, level);
            processed = true;
        }
    }        
...
{
    var axis, index, level, elem, newCellIndex, newElement, newIndex;
    ...
    switch (keyCode)
    {
        case DvtDataGrid.LEFT_KEY:
            if (axis === 'column' && index > 0)
            {
                ...
                else
                {
                    ...
                    {
                        newElement = this._getColumnHeaderByIndex(this._getAttribute(elem['parentNode'], 'start', true) - 1, level);
                ...
                this._setActiveHeader(newIndex, this._getKey(newElement), axis, level);
            }
            else if (axis === 'row' && level > 0)
            {
                newElement = level === this.m_rowHeaderLevelCount - 1 ? elem['parentNode']['firstChild'] : elem['parentNode']['parentNode']['firstChild'];
                newIndex = this._getAttribute(newElement['parentNode'], 'start', true);
                //moving down a level in the header
                this._setActiveHeader(newIndex, this._getKey(newElement), axis, level - 1);
            }
            break;
        case DvtDataGrid.RIGHT_KEY:
            if (axis === 'row' )
            {
                ...
                else
                {
                    ...
                    //moving down a level in the header
                    this._setActiveHeader(index, this._getKey(newElement), axis, level + 1);
                }
            }
            else
            {
        ...
        case DvtDataGrid.UP_KEY:
            ...
            {
                newElement = level === this.m_columnHeaderLevelCount - 1 ? elem['parentNode']['firstChild'] : elem['parentNode']['parentNode']['firstChild'];
                newIndex = this._getAttribute(newElement['parentNode'], 'start', true);
                //moving down a level in the header
                this._setActiveHeader(newIndex, this._getKey(newElement), axis, level - 1);
            }            
...
DvtDataGrid.prototype.handleSelectionKeyDown = function(event)
{
    ...
    if (this.isActionableMode())
    {
        ...
        // if arrow key is used and there's no focusable element inside the cell
        // it breaks the actionable mode
        if (this.isArrowKey(keyCode) && !this._isContainFocusableElement(cell))
        {
            this.setActionableMode(false);
...
DvtDataGrid.prototype.selectAndFocus = function(index, event, augment)
{
    ...
    this.setActive(index, event, this._selectAndFocusActiveCallback.bind(this, index, event, augment));
};

/**
 * Continue to selectAndFocus
 * @param {Object} index - the end index of the selection.  
 ...
 * @private  
 */
DvtDataGrid.prototype._selectAndFocusActiveCallback = function(index, event, augment)
{        
...
DvtDataGrid.prototype.resizeColHeight = function(newElementHeight, heightChange)
{
    if (heightChange != 0)
    {
        ...
        this.m_columnHeaderLevelHeights[level] = newElementHeight;
...
DvtDataGrid.prototype.resizeRowWidth = function(newElementWidth, widthChange)
{
    ...
    {
        var level = this.getHeaderCellLevel(this.m_resizingElement);
        this.m_rowHeaderLevelWidths[level] = newElementWidth;
...
/**
 * Determine what the minimum height for the resizing element is
...
{
    var index, level, elem, minValue, extent, currentDimensionValue, inner, innerDimensionValue;
    ...
    if ((axis === 'column' && (this.m_columnHeaderLevelCount - 1 === level|| dimension === 'height')) ||
        (axis === 'row' && (this.m_rowHeaderLevelCount - 1 === level || dimension === 'width')))
    {
        return minValue;
    }
    index = this.getHeaderCellIndex(elem);
...
DvtDataGrid.prototype.resizeColumnWidthAndShift = function(widthChange)
{
    ...
    if (this.m_columnHeaderLevelCount > 1 && this.m_resizingElement === this.m_resizingElement['parentNode']['firstChild'])
    {
    ...
    if (this.m_databody['firstChild'] != null)
    {
        ...
        for (i = 1; i < databodyRows.length; i++)
        {
...
DvtDataGrid.prototype.resizeRowHeightAndShift = function(heightChange)
{
    ...
    //get the index of the header, if it is a nested header make it the last child index
    index = this.getHeaderCellIndex(this.m_resizingElement);
    if (this.m_rowHeaderLevelCount > 0 && this.m_resizingElement === this.m_resizingElement['parentNode']['firstChild'])
    {
        index += this._getAttribute(this.m_resizingElement['parentNode'], 'extent', true) - 1;
    }
    ...
    if (this.m_databody['firstChild'] != null)
    {
        databodyRows = this.m_databody['firstChild']['childNodes'];
        // +1 for resizer
        row = databodyRows[index - this.m_startRow + 1];
        newHeight = this.getElementHeight(row) + heightChange;
        this.setElementHeight(row, newHeight);
        // +1 for resizer, +1 for the header we just did
        for (i = index - this.m_startRow + 2; i < databodyRows.length; i++)
        {
            row = databodyRows[i];
...
{
    var groupings, i, grouping, isHeader, headerLevel, headers, newDir, j;
    ...
    for (i = 0; i < groupings.length; i++)
    {
        ...
        if (!isHeader)
        {
            ...
            {
                // if level is higher then we need to adjust the dir of all the ehaders under that group
        ...
        else
        {
            ...
            if (headerLevel === level)
            {
                if (axis === 'column')
                {
                    this.setElementHeight(grouping, this._getColumnHeaderLevelHeight(level, null));
                }
                else
                {
                    this.setElementWidth(grouping, this._getRowHeaderLevelWidth(level, null));
                }
            }
...
DvtDataGrid.prototype.handleCellsFetchSuccessForSort = function(newRowHeaderElements, cellSet, cellRange)
{
    ...
    if (!duration || duration == 0 || !this.m_utils.supportsTransitions() || rowCount === 1 || (this.m_rowHeaderLevelCount > 1 && this.m_rowHeaderLevelCount != null))
    {
        if(newRowHeaderElementsFragment.childNodes.length > 1)
        {
            headerContent = this.m_rowHeader['firstChild'];
            this.m_utils.empty(headerContent);
            for (i = 1; i < newRowHeaderElementsFragment.childNodes.length; i++)
            {
                newRowHeaderElementsFragment.childNodes[i].style.top = oldRowElements.childNodes[i].style.top;
            }
...
{
    var dataKeySet, i, key;

    dataKeySet = [];    
    for(i = 0; i < initialData.childNodes.length; i++) //vvc12
    {
        key = this._getKey(initialData.childNodes[i]);
        if (key != null)
        {
            dataKeySet.push(key);
        }
...
DvtDataGrid.prototype.processSortAnimationToPosition = function(duration, delay_offset, timing, oldRowHeaderElements, newRowHeaderElements, oldElementSet, newElementSet)
{
    var me, pos, oldLength, oldSet, newSet, animOrder, restSet, v, gridHeight, colHeaderHeight, gridBodyHeight,
        ...
        i, j, key, k, kk, l, dkey, delay, order, idx, bottomLimit, topLimit, rowPos, child, colHeader, rowHeaderSupport,
    ...
    //newSet contains the keys
    newSet = this.getDataKeySet(newElementSet);
    oldSet = this.getDataKeySet(oldElementSet);
    //the key attribute is not cloned so add it
    for (i = 0; i < newSet.length; i++)
    {
        this._setKey(newElementSetClone.childNodes[i+1], newSet[i]);
        this._setKey(newRowHeaderElementsClone.childNodes[i+1], newSet[i]);        
    }
    animOrder = [];
    restSet = [];
    rowsForAppend = [];
    ...
    rowHeight = this.calculateRowHeight(this._findRowByKey(oldSet[0])); 
    ...
    if (oldRowHeaderElements.childElementCount == oldLength && newRowHeaderElementsClone)
    {
    ...
    if (rowHeaderSupport)
    {
        for (i = 0; i < newRowHeaderElements.childNodes.length; i++) //vvc12
        {
            newRowHeaderElements.childNodes[i].style.top = oldElementSet.children[i].style.top; //vvc12
        }
    }
                ...
                if (animOrder[newSet[j]] > j)
                {
                    ...
                    else
                    {
                        ...
                        if (this.isNumeric(animOrder[newSet[j]]) && animOrder[newSet[j]] > limitRangeInRows[1])
                        {
                            ...
                            this.addTransformMoveStyle(oldElementSet.childNodes[animOrder[newSet[j]] + 1], 0, 0, 'linear', 0, dv * rowHeight, 0); //move it on border
                            if (rowHeaderSupport)
                            {
                                me.addTransformMoveStyle(oldRowHeaderElements.childNodes[animOrder[newSet[j]] + 1], 0, 0, 'linear', 0, (dv + 1) * rowHeight, 0);
                            }
                        }
                    ...
                    if (animOrder[newSet[j]] < j)
                    {
                        ...
                        else
                        {
                            ...
                            if (this.isNumeric(animOrder[newSet[j]]) && animOrder[newSet[j]] < limitRangeInRows[0])
                            {
                                ...
                                if (rowHeaderSupport)
                                {
                                    me.addTransformMoveStyle(oldRowHeaderElements.childNodes[animOrder[newSet[j]] + 1], 0, 0, 'linear', 0, dv * rowHeight, 0);
                                }
    ...
    for (key in restSet)
    {
        if (restSet.hasOwnProperty(key))
        {
            ...
            if (rowPos < bottomLimit)
            {
                if (rowPos > topLimit)
                {
                    child = newElementSetClone.childNodes[idx+1];
                    rowsForAppend.push(child);
                    if (rowHeaderSupport)
                    {
                        rowHeadersForAppend.push(newRowHeaderElementsClone.childNodes[idx + 1]);
                    }
                }
            }
        }
    ...
    {
        dkey = this._getKey(rowsForAppend[k]);
        if (restSet.hasOwnProperty(dkey))
        {
            order = restSet[dkey];
                this.addTransformMoveStyle(rowsForAppend[k], 0, 0, 'linear', 0, limitRangeInPixels[1] - order * rowHeight, 0);
            if (rowHeaderSupport)
            {
                rowHeadersForAppend[k].style.top = rowsForAppend[k].style.top;
                me.addTransformMoveStyle(rowHeadersForAppend[k], 0, 0, 'linear', 0, limitRangeInPixels[1] - order * rowHeight, 0);
            }
        }
    ...
    oldElementSet.childNodes[oldElementSet.childElementCount - 1].addEventListener('transitionend', function()
    {
        //TODO: Chris: if the transitions are over now do we need to set timeout? I removed it and saw no difference
        setTimeout(function()
        {
            if (rowHeaderSupport)
            {
                ...
                me.m_utils.empty(headerContent);
                headerContent.appendChild(newRowHeaderElements);
                //TODO: Chris: I think this will break the virtual case where startRowHeader is not 0
                me.m_startRowHeader = 0;
            }
            var databodyContent = me.m_databody['firstChild'];
            me.m_utils.empty(databodyContent);
            databodyContent.appendChild(newElementSet);

            // restore active index
            me._restoreActive();
            me.m_isSorting = false;
            me._fireSortEvent();
            me._doDelayedSort();
        }, 100);
    }, false);

    //main animation loop, +1 for resizer
    for (l = 1; l < oldElementSet.childElementCount; l++)
    {
        delay = delay_offset * l + "ms";
        if (l < oldLength) //animate rows from the old set 
        {
            pos = parseInt(animOrder[oldSet[l - 1]].split('_')[1], 10) * rowHeight;
            if (pos < limitRangeInPixels[1] - (l-1) * rowHeight) //row is in the visible area
            {
                this.addTransformMoveStyle(oldElementSet.childNodes[l], duration / 2 + "ms", delay, timing, 0, pos, 0);
                if (rowHeaderSupport)
                {
                    me.addTransformMoveStyle(oldRowHeaderElements.childNodes[l], duration / 2 + "ms", delay, timing, 0, pos, 0);
                }
            }
            else //row is out of the visible area, move it on the border of visible area
            {
                pos = limitRangeInPixels[1] - (l - 2) * oldElementSet.childNodes[1].clientHeight;
                this.addTransformMoveStyle(oldElementSet.childNodes[l], duration / 2 + "ms", delay, timing, 0, pos, 0);
                if (rowHeaderSupport)
                {
                    me.addTransformMoveStyle(oldRowHeaderElements.childNodes[l], duration / 2 + "ms", delay, timing, 0, pos, 0);
        ...
        { //animate rows added to the old set from the new one to its original positions
            this.addTransformMoveStyle(oldElementSet.childNodes[l], duration / 2 + "ms", delay, timing, 0, 0, 0);
            if (rowHeaderSupport)
            {
                me.addTransformMoveStyle(oldRowHeaderElements.childNodes[l], duration / 2 + "ms", delay, timing, 0, 0, 0);
            }
        }
    } 
};
...
DvtDataGrid.prototype._restoreActive = function()
{
    if (this.m_active)
    {
        // get the updated index of where the active key is
...
DvtDataGrid.prototype._restoreActiveCallback = function(indexes)
{
    ...
    // if high watermark scrolling we only highlight if it was in the viewport
    // if virtual we will scorll to the position
    if (!this._isHighWatermarkScrolling() || this._isInViewport(cellIndex) != -1)
    {
        // make sure it's visible
